...
import { IHealthHistory, IHealthHistoryService } from "../../interfaces/IHealthHistory";
...
class HealthHistoryService implements IHealthHistoryService {
    ...
    async updateHealthHistory(patientId: string, updateData: Partial<IHealthHistory>): Promise<IHealthHistory> {
        try {
            const updatedData = await this.healthHistoryRepo.upateHealthHistory(patientId, updateData)
