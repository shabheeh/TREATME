...
export class SocketService implements ISocketService {
    ...
    private authenticateSocket  = async (socket: Socket, next: (err?: ExtendedError) => void) => {
        try {
            const token = socket.handshake.auth.token || socket.handshake.query.token;

            if (!token) {
                const err = new Error("Authentication failed: No token provided") as ExtendedError;
                err.data = { status: 401 };
                return next(err);
              }
            ...
            const err = new Error(`Authentication failed: ${error instanceof Error ? error.message : "Invalid token"}`) as ExtendedError;
            err.data = { status: 401 };
            return next(err);
        }
    }
    ...
    private async joinUserChats(userId: string, socket: Socket): Promise<void> {
        try {
            ...
            logger.error(error instanceof Error ? error.message: "Error joining user chats");
        }
    }
    ...
    private async handleReadMessages(socket: Socket, chatId: string): Promise<void> {
        try {
            const userId = socket.data.user.id;
    ...
    // public methods for sending events from outside the socket servic
