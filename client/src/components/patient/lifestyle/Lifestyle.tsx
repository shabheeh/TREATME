...
const Lifestyle: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchLifestyle = async () => {
      try {
        setLoading(true);
        const result = await healthProfileService.getLifestyle(currentPatient._id);
        setQuestions([
          {
            id: 'doExercise',
  ...
  const handleAnswer = async (questionId: string, newAnswer: boolean) => {
    try {
      if (!currentPatient) return;
      await healthProfileService.updateLifestyle(currentPatient._id, { [questionId]: newAnswer });
      setQuestions((prev) =>
