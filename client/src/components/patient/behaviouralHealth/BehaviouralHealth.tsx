...
const BehavioralHealth = () => {
  ...
  const fetchMentalHealth = async () => {
    ...
    setLoading(true);
    try {
      const result = await healthProfileService.getBehaviouralHealth(
        currentPatient._id
      );
            ...
            {MENTAL_HEALTH_CONDITIONS.map((condition) => (
              <Grid item key={condition}>
                <Chip
                  label={condition}
                  onClick={() =>
                    isEditing && handleArrayToggle("conditions", condition)
                  }
                  color={
            ...
            {COPING_MECHANISMS.map((mechanism) => (
              <Grid item key={mechanism}>
                <Chip
                  ...
                  onClick={() =>
                    isEditing &&
                    handleArrayToggle("copingMechanisms", mechanism)
                  }
                  color={
                    editedData &&
                    editedData.copingMechanisms?.includes(mechanism)
                      ? "primary"
                      : "default"
                  }
                  sx={{
            ...
            {SUPPORT_SYSTEM_OPTIONS.map((support) => (
              <Grid item key={support}>
                <Chip
                  label={support}
                  onClick={() =>
                    isEditing && handleArrayToggle("supportSystem", support)
                  }
                  color={
                    editedData && editedData.supportSystem?.includes(support)
                      ? "primary"
                      : "default"
                  }
                  sx={{
